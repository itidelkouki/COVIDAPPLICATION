...
try {
            const profiles=await Profile.find().populate("doctorId")
            res.json({profiles})
        } catch (error) {
    ...
    getProfileById:async (req,res)=>{
        ...
        try {
            const profile = await Profile.findById(req.params.id).populate('doctorId');
            console.log(profile);
            if(!profile) {
...
getCurrentProfile : async (req, res) => {
    try {
        ...
        res.status(500).send("Server error XXXXXXXXXXX");
    }
},}
